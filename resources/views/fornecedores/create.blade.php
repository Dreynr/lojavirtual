<x-app-layout>
    ...
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-md shadow">
        ...
        <form action="{{ url('fornecedores/new') }}" method="POST">
            @csrf
            ...
            <div>
                ...
                <x-text-input class="w-full" type="text" name="nome_razao" value="{{ old('nome_razao') }}" />
            </div>
            
            <div>
                <x-input-label for="cpf_cnpj" :value="__('CPF/CNPJ')" />
                <x-text-input class="w-full" type="text" name="cpf_cnpj" value="{{ old('cpf_cnpj') }}" />
            </div>

            <div>
                <x-input-label for="telefone" :value="__('Telefone')" />
                <x-text-input class="w-full" type="text" name="telefone" value="{{ old('telefone') }}" />
            </div>
